...
class Poller:
    ...
    def __init__(self, bot: commands.Bot, client): #TODO somehow fix the type checking
        self.bot = bot
        self.client = client
    
    async def initialize_club(self, tag: str):
        club_data: dict = (await self.client.get_club(tag, use_cache=False))
        club_members = {}
        tasks = []
        for i, member_data in enumerate(club_data['members']):
            ...
            # k,v = await self.initialize_player(member_data, i+1)
            # club_members[k] = v
    ...
    async def initialize_player(self, player_data: dict, rank: int):
        ...
        if len(battle_log_data) > 0: #in case log wipes
            ...
            for battle_data in battle_log_data:
                if args := Poller.is_valid_battle(battle_data):
                    battle = Battle(*args)
                    for i in range(2):
    ...
    @staticmethod
    def is_valid_battle(battle: dict) -> Union[bool, tuple]:
